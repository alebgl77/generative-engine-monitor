...
import { iqr, mad } from "@/lib/scoring/stats";
import type {
  SampleFeatures,
  SampleScoreResult,
  ScoreContribution,
...
/**
 * Scoring v3 — released. Never edited: every score already computed under "v3"
 * must stay reproducible. New behaviour goes in a new version file.
 *
 * Two properties separate it from its predecessor: the brand counts as present
 * only on a match it was actually spelled by, and the citation rule reads what
 * the provider retrieved rather than every URL the answer happens to contain.
 */
...
/** Same role for the interquartile range. */
...
/**
 * 0..1 consistency indicator over a set of sample scores.
 *
 * The two spread estimators are read together and the widest wins: the IQR does
 * not vanish under ties, which is the regime a three-sample cell lives in.
 */
export function stabilityV3(xs: number[]): number {
  const spread = Math.max(MAD_TO_SIGMA * mad(xs), iqr(xs) / IQR_TO_SIGMA);
  return clamp(1 - spread / STABILITY_SPREAD_CAP, 0, 1);
}
...
function scoreSample(features: SampleFeatures): SampleScoreResult {
  ...
  /*
   * Presence is a headline figure — it becomes brandPresenceRate — so it rests
   * on a match the answer spelled out. A fuzzy match is a hypothesis about a
   * token, and it carries a fraction of the confidence of the others.
   */
  const brandPresent = brandMentions.some((m) => m.mentionType !== "APPROXIMATE");
  ...
  /*
   * Only what the provider retrieved is evidence of retrieval. A URL the model
   * typed into its answer is part of the answer, and counting it would make a
   * parametric-looking answer indistinguishable from a grounded one.
   */
  const retrieved = features.citations.filter((c) => c.sourceKind === "NATIVE");
  const brandCitations = retrieved.filter((c) => c.isBrandDomain);
   ...
   */
  const citationApplicable = brandPresent && features.mode === "GROUNDED" && retrieved.length > 0;
  ...
  if (citationApplicable) {
    citation = makeRule(
      ...
      brandDomainCited
        ...
        : {
            citationIds: retrieved.map((c) => c.normalizedUrl),
            note: `Aucune des ${retrieved.length} sources retournées par le fournisseur n'appartient au domaine de la marque.`,
          }
    );
  } else {
    citation = makeRule(
...
export const v3: ScoringVersion = {
  version: "v3",
