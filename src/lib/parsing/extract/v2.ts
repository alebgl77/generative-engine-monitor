 ...
 * Extraction v2.
 *
 * Released and therefore frozen: mentions stored under "v2" must stay
...
function entityOccurrences(
  ...
  candidates.sort(
    (a, b) => MENTION_PRIORITY[a.type] - MENTION_PRIORITY[b.type] || a.start - b.start
  );
...
function buildMentions(
  ...
  entities.forEach((entity, inputIndex) => {
    const occurrences = entityOccurrences(entity, folded, text.length);
    if (occurrences.length > 0) found.push({ entity, inputIndex, occurrences });
  });
...
export const extractorV2: Extractor = {
  version: "v2",
